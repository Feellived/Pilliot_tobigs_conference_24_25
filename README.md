...
알약 이미지에서 시작해 약물 인식, 속성 분류, 상호작용 해석, 복약 안내까지 연결하는 AI 기반 복약 지원 프로젝트입니다.

이 저장소는 그중에서도 현재 가장 앞단에 있는 컴퓨터 비전 파이프라인, 즉 `탐지 -> crop 생성 -> 분류` 실험을 중심으로 정리한 연구용 베이스라인 레포입니다.

**팀**: 유주형 · 한수영 · 김지우 · 조윤수  
**프로젝트 성격**: ToBigs Conference 프로젝트 / 실험 중심 프로토타입
...
아직 이 레포에 본격적으로 포함되지 않은 영역은 다음과 같습니다.
...
이 저장소는 로컬 단독 실행형 패키지보다는, 다음 외부 자원과 함께 쓰는 실험 레포에 가깝습니다.
...
그래서 노트북을 그대로 실행하려면 데이터 경로, Drive 구조, DB 접근 환경이 맞아야 합니다. 공개 레포만 clone해서 바로 재현되는 구조는 아직 아닙니다.
...
즉 Pillot은 "알약 이미지를 맞히는 모델"이 아니라, "복약 맥락에서 의미 있는 판단을 돕는 시스템"을 목표로 합니다.
...
하지만 반대로, detection-crop-classification의 병목을 단계별로 추적할 수 있다는 점에서 초기 연구 구조로는 장점이 큽니다.

---

## 10. 앞으로의 확장 방향

다음 단계에서 확장하려는 방향은 아래와 같습니다.

- YOLO crop 기반 fine-grained pill ID classification
- OCR 기반 각인 정보 결합
- 속성 분류와 식별 모델의 역할 정리
- DUR/공공 약물 DB 매핑 자동화
- 상호작용 분석과 사용자용 복약 안내까지 통합

---

## 11. 한눈에 보기

외부인이 이 저장소를 처음 본다면, 아래처럼 이해하면 됩니다.
